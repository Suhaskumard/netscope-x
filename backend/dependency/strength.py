"""Dependency strength estimation (spec Phase 51, FR-1.26).

FR-1.26 names five signals: frequency, persistence, directionality, temporal relationships, and
traffic characteristics. Temporal relationships is deliberately excluded here --
`docs/architecture/algorithm_selection.md` section 6 parenthesizes it as "spec Phase 52" in the
very sentence describing the combined scoring function, and `DependencyEdge.temporal_precedence_score`
(`backend/app/models/dependency.py`) already has `default=0.0` with its own docstring tagged "spec
Phase 52" -- this module never sets it.

The remaining four signals all reuse already-built evidence rather than re-deriving it:
...
term (frequency) at full weight, three secondary signals (persistence, directionality, traffic
characteristics) each scaled by one shared, uniformly-applied strength constant -- uniform because
no empirical basis yet justifies weighting one signal above another (that is Phase 68's job).
...
def estimate_dependency_strength(
    ...
    `temporal_precedence_score` is always left at its schema default `0.0` -- Phase 52's job, never
    set here.
